...
export const ContentHeader: React.FC<ContentHeaderProps> = ({
  ...
  return (
    <div className={cn(centered && 'text-center', className)}>
      {eyebrow && (
        <p className={cn('text-sm font-semibold uppercase tracking-wide text-indigo-600 dark:text-indigo-400', eyebrowClassName)}>
          {eyebrow}
        </p>
      )}
      ...
      {subtitle && (
        <p className={cn('mt-4 text-lg text-gray-600 dark:text-gray-300', subtitleClassName)}>
          {subtitle}
        </p>
      )}
    </div>
