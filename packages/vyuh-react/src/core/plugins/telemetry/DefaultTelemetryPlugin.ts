...
export class DefaultTelemetryPlugin extends TelemetryPlugin {
  ...
  async reportError(
    ...
    const promises = this.providers.map(provider => 
      provider.reportError(error, options)
    );
  ...
  async startTrace(
    ...
    const traces = await Promise.all(
      this.providers.map(provider => provider.startTrace(name, operation, options))
    );
    ...
    return {
      ...
      startChild: async (childName: string, childOperation: string, childOptions?: any) => {
        const childTraces = await Promise.all(
          traces.map(trace => trace.startChild(childName, childOperation, childOptions))
        );
        return {
          stop: async () => {
            await Promise.all(childTraces.map(childTrace => childTrace.stop()));
          }
        };
      }
    };
  ...
  log(
    ...
    this.providers.forEach(provider => 
      provider.log(message, level, params)
    );
  }
}