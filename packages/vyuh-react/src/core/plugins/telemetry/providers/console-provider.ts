...
export class ConsoleProvider implements TelemetryProvider {
  ...
  async reportError(
    ...
    const stackTrace = options?.stackTrace || (error instanceof Error ? error.stack : '');
  ...
  async startTrace(
    ...
    return {
      stop: async () => {
        ...
        console.log(`[TRACE:END] ${name} - ${operation} (${duration.toFixed(2)}ms)`);
      },
      startChild: async (
        childName: string,
        ...
        console.log(`[TRACE:CHILD:START] ${childName} - ${childOperation} (${childLevel})`);
        
        return {
          stop: async () => {
            const childDuration = performance.now() - childStartTime;
            console.log(`[TRACE:CHILD:END] ${childName} - ${childOperation} (${childDuration.toFixed(2)}ms)`);
          }
        };
      }
    };
